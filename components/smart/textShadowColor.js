...
//actions
import {changeTextShadowColor} from '../../src/actions/changeTextShadowActions';
import {renderTextShadowDesign,renderTextShadowColor} from '../../src/actions/renderOrNotActions';
import dimensionsObj from '../../dimensions';
class TextShadowColor extends Component {
    constructor(props) {
        super(props);
      ...
      render() {
        if(!this.props.renderOrNotReducer.renderTextShadowColor)
            return null;
        else
            return (
            <View style={styles.container}>
                <View style={styles.closeBar}>
                    <TouchableOpacity style= {{alignItems: 'center', justifyContent: 'center',width:'100%', borderBottomColor:'gray', borderBottomWidth: 0.6}} 
                        onPress={() => this.props.renderTextShadowColor() }>
                    ...
                    <ScrollView contentContainerStyle={{alignItems:'center',justifyContent:'center'}}horizontal={true}>
                        {colors[this.state.index].colorSpanArray.map((color,index) =>(
                            <TouchableOpacity key={color} onPress={() => {
                                console.log("haydar");
                                this.props.changeTextShadowColor(color)
                            }}>
